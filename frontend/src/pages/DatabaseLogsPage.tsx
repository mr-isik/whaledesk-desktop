...
import {
  ...
  DeleteRequest,
  GetRequestHistory,
} from "../../wailsjs/go/bindings/DatabaseBinding";
...
export default function DatabaseLogsPage() {
  ...
  const handleClear = async () => {
    if (
      confirm(
        "Are you sure you want to permanently clear the request database history logs?",
      )
    ) {
      await ClearHistory();
      fetchLogs();
      ...
      >
        <div>
          ...
          <p className="page-subtitle" style={{ marginBottom: 0 }}>
            SQLite logs of HTTP transaction requests sent through whaledesk
          </p>
        </div>
        <button
        ...
        <div style={{ overflowY: "auto", flex: 1 }}>
          {loading ? (
            ...
            >
              ...
              >
                Send requests using the API Studio and their metadata history
                logs will populate here automatically.
              </p>
            </div>
          ) : (
            logs.map((log) => {
